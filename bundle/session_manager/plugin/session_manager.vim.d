...
if !exists('g:sessions_data_path')
    let g:sessions_data_path = "~/.vim_sessions/"
endif
if !exists('g:session_save_before_load')
    let g:session_save_before_load = "yes"
endif
...
function! SessionGetFileFullPath()
        return glob(g:sessions_data_path) . SessionGetFileName()
endfunction
...
function! SessionNew()
    ...
    call SessionCreate(glob(g:sessions_data_path) . l:ses_file)
...
function! SessionLoad(ses_file)
    if v:this_session != "" && g:session_save_before_load == "yes"
        call SessionSave()
    endif
    let l:ses_file = glob(g:sessions_data_path) . a:ses_file
    try
        exec "so ". l:ses_file
    catch E445
...
function! SessionManagerOpen()
    ...
    for sesfile in split(glob(g:sessions_data_path."*"), "\n")
