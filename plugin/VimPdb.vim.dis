...
function! PdbMapKeyboard()
	...
	map <buffer> <silent> <C-S-F8> :call PdbContinueUntilReturn()<CR>

	map <buffer> <silent> <C-F9> :call PdbMoveUpInStackFrame()<CR>
	map <buffer> <silent> <C-F10> :call PdbMoveDownInStackFrame()<CR>

	map <buffer> <silent> <C-F6> :call PdbSetFocusToCurrentDebugLine()<CR>
	map <buffer> <silent> <C-S-F6> :call PdbJumpToCurrentLine()<CR>

	map <buffer> <silent> <F11> :call PdbToggleBreakpointOnCurrentLine()<CR>
	map <buffer> <silent> <C-F11> :call PdbToggleConditionalBreakpointOnCurrentLine()<CR>
	map <buffer> <silent> <S-F11> :call PdbToggleTemporaryBreakpointOnCurrentLine()<CR>
	map <buffer> <silent> <C-S-F11> :call PdbClearAllBreakpointsInCurrentFile()<CR>
	map <buffer> <silent> <C-A-S-F11> :call PdbClearAllBreakpoints()<CR>

	map <buffer> <silent> <C-F12> :call PdbPrintBreakpointConditionOnCurrentLine()<CR>

	map <buffer> <silent> <C-F4> :call PdbEvalCurrentWord()<CR>
	map <buffer> <silent> <C-S-F4> :call PdbEvalCurrentWORD()<CR>

	map <buffer> <silent> <C-F3> :call PdbEvalExpression()<CR>
	map <buffer> <silent> <C-S-F3> :call PdbExecStatement()<CR>
